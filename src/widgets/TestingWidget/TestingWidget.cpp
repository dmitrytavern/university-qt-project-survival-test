...
const std::vector<TestingQuestionData> questionsData = {
    ...
    {"6. З кінцевими користувачами продукту налагоджено двосторонню зв'язок", "На этапе разработки продукта было установлено взаимодействие с конечными пользователями и обратная связь была организована."},
    ...
    {"8. До списку завдань проекту включені «вторинні» завдання.", "В проекте были учтены все необходимые задачи, даже те, которые могут показаться менее значимыми или могут быть легко забыты"},
    ...
    {"18. Усі зацікавлені сторони можуть отримати необхідну інформацію про хід проекту", "На проекте должна быть реализована система отчетности, которая позволяет предоставлять информацию о ходе работы проекта всем заинтересованным сторонам в удобной форме."},
